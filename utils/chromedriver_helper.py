...
class ChromeDriver:
    ...
    def get_driver(self, chrome_data=_chrome_data, driver_path=_driver_path):
        with open(chrome_data) as f:
            version = f.readline()
        version_path = os.path.join(driver_path, version)
        self.get_file(version=version, path=driver_path)
        return os.path.join(version_path, "chromedriver.exe")

    def get_file(self, version, path=_driver_path, mirror=_mirror, ):
        ...
        elif os.path.exists(os.path.join(version_path, "chromedriver.exe")):
            return
        try:
    ...
    def get_android_driver(self, version, driver_path=_driver_path):
        self.get_file(version=version, path=driver_path)
        version_path = os.path.join(driver_path, version)
        return os.path.join(version_path, "chromedriver.exe")