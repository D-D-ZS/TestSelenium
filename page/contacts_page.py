...
class ContactsPage(BasePage):
    ...
    def get_member(self, username):
        ...
        while True:
            # current_page, total_page = self.update_page()
            logging.info(f'get_member===:当前页码:{current_page+1}')
            self.wait_for_click((By.CSS_SELECTOR, '.js_operationBar_footer>.js_add_member'))
            member_tr = self.finds(By.CSS_SELECTOR, '.member_colRight_memberTable_tr')
            logging.info(f'get_member===:第一次获取 member 列表，共有成员{len(member_tr)}个')
            while len(member_tr) <= 0:
                member_tr = self.finds(By.CSS_SELECTOR, '.member_colRight_memberTable_tr')
                logging.info(f'get_member===:第一次获取失败，再次获取，共有成员{len(member_tr)}个')
            loop = 1
            for member in member_tr:
                logging.debug(f'get_member===:读取成员{member}')
                try:
                    ...
                    logging.info(f'get_member===:获取第{loop}个成员名称：{name}')
                    if username == name:
                        logging.info(f'get_member===:成员名称{username}在通讯录中（{name}），返回')
                        return member_info
                except Exception as e:
                    logging.error(f'get_member===:获取成员名称失败 {e}')
                finally:
                    loop += 1
            current_page, total_page = self.update_page()
            if current_page == total_page:
                logging.info(f'get_member===:成员名称{username}不在通讯录中，返回')
                return None
            else:
                self.find(By.CSS_SELECTOR, '.js_has_member>.ww_operationBar:nth-child(1) .js_next_page').click()
                logging.info(f'get_member===:当前页没有找到成员名称{username}，进入下一页')
    ...
    def delete_member(self, username):
        ...
        if member_info is not None:
            ...
            ele.click()
            logging.info(f'delete===:删除用户：{username}')
        else:
